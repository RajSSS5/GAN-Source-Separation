...
def adversarial_loss(y_true, y_pred):
    return th.neg(th.sum(th.log2(y_pred),axis=1))

def segmentation_loss(y_true, y_pred):
    # -sum over array
        # sum over channels
            # Ground truth * log(predicted)
    return th.neg(th.tensordot(y_true, th.log2(y_pred),axes=[[1],[1]]))
...
def contains_voice(fragment, sequence):
    ...
    for i in range(0, len(signal_fragment_magnitude), chunk_length):
        chunks_fragment_energies.append(np.mean(signal_fragment_magnitude[i:i + chunk_length]))

    chunks_sequence_energies = []
    for i in range(0, len(signal_sequence_magnitude), chunk_length):
